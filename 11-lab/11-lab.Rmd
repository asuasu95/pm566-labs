...
Date range is from 2020-01-21 to 2020-10-27.
Range of cases is from 1 to 923828.
Range of deaths is from 0 to 33092.
...
According to the plot, pop_density seems to be correlated with newdeathsper100k with a positive slope.
...
### FINISH THE CODE HERE ###
cv_states %>% filter(state=="Texas") %>% plot_ly(x = ~date, y = ~new_cases, type = "scatter", mode = "lines") %>% 
  add_lines(x=~date,y=~new_deaths,type="scatter",mode="lines")

```

The peak for cases is on Sep 21th.
The peak for deaths is on July 27th.
56 days in between
...
fig <- fig %>% colorbar(title = "CFR May 1 2020", cols = c(0,shadeLimit))
fig <- fig %>% layout(
...
fig <- fig %>% colorbar(title = "CFR Today", cols = c(0,shadeLimit))
fig <- fig %>% layout(
    title = paste('CFR by State as of', Sys.Date(), '<br>(Hover for value)'),
    geo = set_map_details
  )
